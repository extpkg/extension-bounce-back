...
function DebugConsoleKeyDown(e) {
  ...
  try {
    ...
    if (e instanceof Error) o += e.message ? "Error: " + e.message : e;
    else o += "Unknown error";
  }
...
function InitDebug() {
  ...
  debugConsoleTextArea.style =
    "height:30px;width:90%;display:none;color:#FFF;background-color:#000;";
  ...
  debugConsoleDisplayTextArea.style =
    "height:100px;width:90%;display:none;color:#FFF;background-color:#000;";
...
function RenderDebugRects() {
  ...
  for (let d of debugRects) RenderDebugRect(d.pos, d.size, d.color);
  debugRects = [];
}
