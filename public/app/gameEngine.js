...
class Vector2 {
  ...
  Clone(s = 1) {
    return new Vector2(this.x, this.y).Multiply(s);
  }
  Add(v) {
  ...
  ClampLength(length) {
    let l = this.Length();
    return l > length ? this.Multiply(length / l) : this;
  }
  Rotate(a) {
    let c = Math.cos(a);
    let s = Math.sin(a);
  ...
  Lerp(v, p) {
    return this.Add(v.Clone().Subtract(this).Multiply(p));
  }
  DotProduct(v) {
...
class GameObject {
  ...
  Damage(damage) {
    if (this.IsDead() || this.GetDamageTime() < 0.5) return 0;
  ...
  GetLifeTime() {
    return this.lifeTimer.Get();
  }
  GetDamageTime() {
    return this.damageTimer.Get();
  }
  GetDamageFlashPercent() {
    return Clamp(1 - this.GetDamageTime() / this.damageFlashTime, 0, 1);
  }
  IsTouching(object) {
    return this.Distance(object) < object.collisionSize + this.collisionSize;
  }
  IsDead() {
    return !this.health;
  }
...
function EngineInit() {
  ...
  tileMaskCanvasContext.fillRect(
    0,
    0,
    tileMaskCanvas.width,
    tileMaskCanvas.height,
  );
...
function EngineUpdate() {
  ...
  mousePosWorld
    .Copy(mousePos)
    .Subtract(mainCanvasSize.Clone(0.5))
    .Multiply((1 / cameraScale) * tileSize)
    .Add(cameraPos);
...
function SetCanvasTransform(pos, size, angle = 0, height = 0) {
  ...
  if (shadowRenderPass)
    mainCanvasContext.transform(
      1,
      0,
      shadowSkew,
      shadowScale,
      -shadowSkew * cameraScale * s.x,
      cameraScale * (1 - shadowScale) * s.y,
    );
  if (angle) mainCanvasContext.rotate(angle);
...
class Level {
  ...
  GetDataFromPos(pos) {
    return this.GetData(pos.x, pos.y);
  }

  GetData(x, y) {
    if (!IsArrayValid(x, y, levelSize)) return new LevelData();
  ...
  IsAreaClear(pos, size, gameObject = 0) {
    ...
    for (let yo = y - size; yo <= y + size; ) {
      for (let xo = x - size; xo <= x + size; ) {
        ...
        if (gameObject) {
          if (gameObject.CollideLevel(data, p)) return 0;
        } else if (data.IsSolid()) return 0;

        if (xo == x + size) break;
        ++xo;
        if (xo > x + size) xo = x + size;
      }

      if (yo == y + size) break;
      ++yo;
      if (yo > y + size) yo = y + size;
    }
    return 1;
  }

  FillCircleObject(pos, r, object) {
    this.FillCircleCallback(
      pos,
      r,
      (d) => (d.object = d.type ? object : d.object),
    );
  }
  FillCircleType(pos, r, type) {
    this.FillCircleCallback(pos, r, (d) => (d.type = type));
  }
  ...
  FillCircleCallback(pos, r, callback) {
    ...
    for (let i = -r; i <= r; i++) {
      ...
      for (let j = pos.y - h; j < pos.y + h; j++) {
        let x = (pos.x + i) | 0;
        let y = j | 0;
        if (!IsArrayValid(x, y, levelSize)) continue;
  ...
  Redraw() {
    ...
    levelCanvas.width |= 0;
    for (let y = 0; y < levelSize; y++)
      for (let x = 0; x < levelSize; x++) this.DrawTileData(x, y);
  }

  Render() {
    // draw the entire level (cached on a canvas) onto the main canvas
    let pos = cameraPos
      .Clone(-cameraScale * tileSize)
